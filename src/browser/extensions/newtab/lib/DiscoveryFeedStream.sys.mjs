 ...
 * file, You can obtain one at http://mozilla.org/MPL/2.0/. */
 ...
 export class DiscoveryStreamFeed {
   ...
   getUserRSSFeedURLs () {
     try {
       var raw = Services.prefs.getCharPref('merino.rss.feeds');
       ...
       feeds.forEach(feed => {
         if(typeof feed === 'string') {
           try {
             new URL(feed); // will throw error if invalid
             exp.push(feed);
           } catch (error) {
             // Only ignore this feed URL
             void(error);
           }
         }
       });
   ...
   async loadExternalData() {
     var k = [];
     // Load RSS
     var rssfu = this.getUserRSSFeedURLs();
     var d;
     for(var i = 0; i < rssfu.length; i++) {
       try {
         d = await this.grabRSSFeedAsJSON(rssfu);
     ...
     // Finalize
     var finale = this.formatExternalContent(k);
     console.log(finale);
     return finale;
   }
 
   formatExternalContent (sortables) {
     var kopipe = [];
     var sorting = Array.from(sortables);
     sorting.sort((a, b) => a.date.getTime() - b.date.getTime());
     sorting.forEach(sortable => {
       if(sortable.type === 'rss') {
         kopipe.push({
           corpusItemId: Services.uuid.generateUUID(),
           scheduledCorpusItemId: Services.uuid.generateUUID(),
           title: sortable.item.title,
           excerpt: sortable.item.description,
           url: sortable.item.link,
           imageUrl: sortable.item.img,
           topic: 'rss',
           tileId: 2763548010322901,
           isTimeSensitive: true,
           iconUrl: null,
           receivedRank: 0,
           publisher: sorting.feed.title,
           feed: {
             name: sorting.feed.title,
             url: sorting.feed.link
           }
         })
       } else if(sortable.type === 'merino') {
         // Already merino-compatible
         kopipe.push(sortable.merino);
       }
     });
     return {
   ...
   async fetchFromEndpoint(rawEndpoint, options = {}) {
     ...
     try {
       ...
       const fetchPromise = this.loadExternalData();
       // istanbul ignore next
    ...
    * @param {string} url 
    */
   async grabRSSFeedAsJSON(url) {
     var raw = await (await fetch(url)).text();
     var parser = new DOMParser();
     var xdom = parser.parseFromString(raw, 'text/xml');
     if(xdom.documentElement.tagName.toLowerCase() != 'rss') {
       // Not a valid RSS document
       throw new SyntaxError('document is not a valid RSS feed');
     };
     var feed = {
       title: xdom.querySelector('channel > title').textContent,
       link: xdom.querySelector('channel > link').textContent,
       description: null,
       items: []
     };
     if(xdom.querySelector('channel > description')) {
       feed.description = xdom.querySelector('channel > description').textContent;
     }
     var items = Array.from(xdom.querySelectorAll('channel > item'));
     items.forEach(xdi => {
       try {
         var item = {
           title: xdi.querySelector('title').textContent,
           link: xdi.querySelector('link').textContent,
           description: xdi.querySelector('description').textContent,
           date: new Date(xdi.querySelector('pubDate').textContent),
           img: null
         };
         var validEnc = [
           'image/png', 'image/jpg', 'image/jpeg', 'image/webp', 'image/gif'
         ];
         if(xdi.querySelector('enclosure')) {
           if(validEnc.includes(
             xdi.querySelector('enclosure').getAttribute('type')
           )) {
             item.img = xdi.querySelector('enclosure').getAttribute('url');
           }
         }
